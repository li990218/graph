...
def process(mat_path, mat_name, grp_scale=10, clst_rate=10, diag_ratio=512, mode='plain'):
    mat = scipy.io.loadmat(mat_path + '/' + mat_name + '.mat')['Problem'][0][0]['A']
    print('========== Process in [{}] mode ==========='.format(mode))
    print(type(mat))
    if mode == 'plain':
        plain_process(mat, mat_path, mat_name, grp_scale)
    elif mode == 'norm':
        normal_process(mat, mat_path, mat_name, grp_scale, clst_rate)
    elif mode == 'diag':
        diag_process(mat, mat_path, mat_name, grp_scale, clst_rate, diag_ratio)


def normal_process(mat, mat_path, mat_name, grp_scale=10, clst_rate=10):
    rc = RowCluster()
    rc.setMatrix(mat)
    print('[matrix]')
    ...
    vs.plotSingle(bitmap2, plt_name, mat_path)
    plt_name = mat_name + '_pre_s{}'.format(grp_scale)
    vs.plotSingle(bitmap1, plt_name, mat_path)


def plain_process(mat, mat_path, mat_name, grp_scale=10):
    rc = RowCluster()
    rc.setMatrix(mat)
    rc.setGroup(grp_scale)
    bitmap1, _ = rc.get_bitmap(rc.A)
...
def diag_process(mat, mat_path, mat_name, grp_scale=10, clst_rate=10, diag_ratio=20):
    n = mat.shape[0]
    c = mat.shape[1]
    A = mat.tocsr()
    nnz = A.data
    ...
    plain_process(A1, mat_path, mat_name+'_diag', grp_scale=grp_scale)
    normal_process(A2, mat_path, mat_name+'_non_diag', grp_scale=grp_scale, clst_rate=clst_rate)
